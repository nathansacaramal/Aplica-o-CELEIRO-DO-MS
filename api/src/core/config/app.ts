...
export const createApp = (): Application => {
  ...
  // MEDIA_STORAGE=local: serve os arquivos gravados em disco (produção usa S3 direto).
  // Precisa vir antes de setupMiddlewares: o middleware `contentType` seta um Content-Type
  // padrão (application/json) quando ainda não há um definido, e express.static (via `send`)
  // só define o Content-Type correto do arquivo se o header ainda não tiver sido setado.
